...
en5SHADER EN5_RndShaders[EN5_MAX_SHADERS];
INT EN5_RndShadersSize = 0;
 ...
 */
static CHAR * EN5_RndShdLoadTextFromFile( CHAR *FileName )
{
  FILE *F;
  CHAR *txt;
  INT flen;
  ...
  fclose(F);
  return txt;
} /* End of 'EN5_RndShdLoadTextFromFile' function */
 ...
 * RETURNS: None.
 */
static VOID EN5_RndShdLog( CHAR *Prefix, CHAR *PartName, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("BIN/SHADERS/EN5{30}SHD.LOG", "a")) == NULL)
    return;
  fprintf(F, "%s/%s.GLSL\n%s\n", Prefix, PartName, Text);
  fclose(F);
} /* End of 'EN5_RndShdLog' function */
...
INT EN5_RndShdLoad( CHAR *ShaderFileNamePrefix )
{
  ...
  for (i = 0; i < NoofS; i++)
  {
    ...
    txt = EN5_RndShdLoadTextFromFile(Buf);
    if (txt == NULL)
    {
      EN5_RndShdLog(ShaderFileNamePrefix, shd[i].Name, "Error load file");
      is_ok = FALSE;
      break;
    }
    /* Create shader */
    ...
    {
      EN5_RndShdLog(ShaderFileNamePrefix, shd[i].Name, "Error create shader");
      free(txt);
      is_ok = FALSE;
      break;
    }
    ...
    if (res != 1)
    {
      ...
      EN5_RndShdLog(ShaderFileNamePrefix, shd[i].Name, Buf);
      is_ok = FALSE;
      break;
    }
  }
  ...
  if (is_ok)
  {
    ...
    {
      EN5_RndShdLog(ShaderFileNamePrefix, "PROG", "Error create program");
      is_ok = FALSE;
    }
    else
    {
      ...
      if (res != 1)
      {
        ...
        EN5_RndShdLog(ShaderFileNamePrefix, "PROG", Buf);
        is_ok = FALSE;
      }
    }
  }
  ...
  return prg;
} /* End of 'EN5_RndShdLoad' function */
...
} /* End of 'EN5_RndShdDelete' function */
...
{
  if (EN5_RndShadersSize >= EN5_MAX_SHADERS)
    return 0;

  strncpy(EN5_RndShaders[EN5_RndShadersSize].Name, ShederFileNamePrefix, EN5_STR_MAX - 1);
  EN5_RndShaders[EN5_RndShadersSize].ProgId = EN5_RndShdLoad(ShederFileNamePrefix);

  return EN5_RndShadersSize++;
} /* End of 'EN5_RndShaderAdd' function */
...
VOID EN5_RndShadersUpdate( VOID )
{
  ...
  {
    EN5_RndShdDelete(EN5_RndShaders[i].ProgId);
    EN5_RndShaders[i].ProgId = EN5_RndShdLoad(EN5_RndShaders[i].Name);
  }
} /* End of 'EN5_RndShadersUpdate' function */
